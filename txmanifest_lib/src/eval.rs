...
///
/// This backed the action-level `validations` block, which was removed with the
/// schema refactor. It is kept because the 3 rules it *did* enforce (asset
/// distinctness in `dex` and `lending`) have no replacement, and this is the
/// starting point for one — see `manifest::tests::removed_legacy_fields_are_rejected`
/// for what was lost. Delete it if the conclusion is that covenant-level enforcement
/// is sufficient.
