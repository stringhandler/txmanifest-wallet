    ...
    if let (Some(wollet), Some(net)) = (&wollet_opt, network_for_asset) {
        ...
        for inp in action.inputs.as_deref().unwrap_or_default() {
                    ...
                    let entropy = if let Some(resolved) = ctx.get_input(&inp.id) {
                        if let Some(hex) = &resolved.issuance_entropy.clone() {
                            match pset_builder::decode_entropy_hex(hex) {
                                ...
                                Err(err) => {
                                    println!(
                                        "  {} Input '{}' entropy decode failed: {err}",
                                        style("[error]").red(),
                                        inp.id
                                    );
                                    collect_inputs_ok = false;
            ...
            if inp.is_wallet_source() {
                ...
                let (inp_params, inp_hints) = apply_utxo_compile_params(
                    &compile_params_map,
                    &compile_param_type_hints,
                    inp_ut,
                );
                ...
                let (inp_params, inp_hints) = apply_site_compile_param_overrides(
                    inp_params,
                    inp_hints,
                    inp.utxo_source.get("compile_params"),
                    action,
                    &compile_param_type_hints,
                    &ctx,
                );
        ...
        if collect_inputs_ok {
            for output in action.outputs.as_deref().unwrap_or_default() {
                ...
                let amount = match &output.amount_sat {
                    None => {
                        if output.optional.unwrap_or(false) || is_change {
                            continue;
                        }
                        if is_op_return {
                            0u64
                ...
                match &output.destination {
                    ...
                    serde_json::Value::Object(m) if m.contains_key("utxo_type") => {
                        ...
                        let out_simf_path = ut
                            ...
                            .unwrap_or_else(|| simf_path.clone());
                        let (out_params, out_hints) = apply_utxo_compile_params(
                            &compile_params_map,
                            &compile_param_type_hints,
                            ut,
                        );
                        ...
                        let conf_label = if confidential {
                            "confidential"
                        } else {
                            "explicit"
                        };
                    ...
                    serde_json::Value::String(dest) => {
                        ...
                        let bpk = addr.blinding_pubkey.map(|pk| {
                            lwk_wollet::elements::bitcoin::PublicKey {
                                inner: pk,
                                compressed: true,
                            }
        ...
        if covenant_inputs.is_empty() {
            ...
            for inp in &covenant_inputs {
                ...
                let (check_params, check_hints) = check_ut
                    ...
                    .unwrap_or_else(|| {
                        (compile_params_map.clone(), compile_param_type_hints.clone())
                    });
                        ...
                        Ok(tx) => {
                            ...
                            if witness_utxos.iter().any(|u| u.is_none()) {
                                ...
                                for (pset_idx, action_inp) in action_inputs.iter().enumerate() {
                                    ...
                                    let leaf_payloads =
                                        match dry_ut.resolve_extra_leaf_payloads(&ctx) {
                                            Ok(p) => p,
                                            Err(e) => {
                                                println!(
                                                    "    {} leaf_payloads for '{}': {e}",
                                    ...
                                    let dry_simf_path = dry_ut
                                        ...
                                        .unwrap_or_else(|| simf_path.clone());
                                    let (dry_params, dry_hints) = apply_utxo_compile_params(
                                        &compile_params_map,
                                        &compile_param_type_hints,
                                        dry_ut,
                                    );
                                    let (dry_params, dry_hints) =
                                        apply_site_compile_param_overrides(
                                            dry_params,
                                            dry_hints,
                                            action_inp.utxo_source.get("compile_params"),
                                            action,
                                            &compile_param_type_hints,
                                            &ctx,
                                        );
...
mod tests {
    ...
    fn dex_make_offer_computes_maker_spk_and_offer_address() {
        ...
        let fields =
            eval_create_instance_fields(ci, &ctx, &manifest_path, &hints, net, false, debug);
