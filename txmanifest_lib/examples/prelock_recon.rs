...
fn main() {
    ...
    let lender_principal_cov = hexs(&covenant::compute_covenant_script_hash(&dir.join("asset_auth.simf"), &p, &h, net, true).unwrap());
    ...
    add(&mut p, &mut h, "FIRST_PARAMETERS_NFT_ASSET_ID", first_nft, "liquid.asset_id");
    add(&mut p, &mut h, "SECOND_PARAMETERS_NFT_ASSET_ID", second_nft, "liquid.asset_id");
    ...
    add(&mut p, &mut h, "LENDER_PRINCIPAL_COV_HASH", &lender_principal_cov, "bytes32");
    add(&mut p, &mut h, "LENDER_NFT_ASSET_ID", lender_nft, "liquid.asset_id");
    let lending_cov = hexs(&covenant::compute_covenant_script_hash(&dir.join("lending.simf"), &p, &h, net, true).unwrap());
    ...
    let params_nft_out = hexs(&covenant::compute_covenant_script_hash(&dir.join("script_auth.simf"), &p, &h, net, true).unwrap());
    ...
    add(&mut p, &mut h, "PRINCIPAL_OUTPUT_SCRIPT_HASH", borrower_out_hash, "bytes32");
    add(&mut p, &mut h, "PARAMETERS_NFT_OUTPUT_SCRIPT_HASH", &params_nft_out, "bytes32");
    add(&mut p, &mut h, "BORROWER_NFT_OUTPUT_SCRIPT_HASH", borrower_out_hash, "bytes32");
    add(&mut p, &mut h, "BORROWER_PUB_KEY", borrower_pubkey, "pubkey");
    let addr = covenant::compute_covenant_address(&dir.join("pre_lock.simf"), &p, &h, &[], net, true).unwrap();
    ...
    println!("pre_lock spk (this wallet, debug=true) = {:x}", addr.script_pubkey());
